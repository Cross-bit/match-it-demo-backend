...
logger.info(`DB2 init: there are ${pool.idleCount} idle clients.`);
...
async function ExecuteTransaction<T>(
  ...
  errorHandler?: errorHanlderDelegate<T>
): Promise<T> {
  const client = await pool.connect();
  const txId = uuidv4();

  logger.debug(`[DB TX ${txId}] BEGIN`);
  ...
  try {
    ...
    logger.debug(`[DB TX ${txId}] COMMIT`);
    return transactionResult;
  } catch (err) {
    await client.query("ROLLBACK");
    logger.error(`[DB TX ${txId}] ROLLBACK`, { error: err });

    const errMsg = errorMessage ?? "Database error occurred";

    if (errorHandler) errorHandler(err as Error, errMsg);
    defaultDatabaseErrorHandler(err as Error, errMsg);

    return Promise.reject(err);
  } finally {
    client.release();
    logger.debug(`[DB TX ${txId}] RELEASE`);
  }
}
