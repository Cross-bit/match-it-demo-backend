...
class HybridGroupRestaurantRecommender:
    ...
    def get_next_round_recommendation(
        self,
        previous_round_votes: Dict[str, List[Vote]],
        exclude_items: List[int]
    ) -> Dict[str, List[Tuple[int, float]]]:

        next_round_recs: Dict[str, List[Tuple[int, float]]] = {}
    ...
    def _get_first_round_recommendations(
        self,
        exclude_items: List[int]
    ) -> Dict[str, List[Tuple[int, float]]]:
        ...
        # sync část — stejná group doporučení pro všechny
        sync_recommendations = self.single_user_recommender.recommend_next_top_k(self.first_round_ratio, list(exclude_items))
        for user_id in self.users_ids:
            next_round_recs[user_id] = list(sync_recommendations)

        # async část — individuální doporučení pro zbytek okna
        remaining = self.window_size - self.first_round_ratio
        logging.info(f"first round remaining (async part): {remaining}")

        if remaining > 0:
            sync_ids = [item_id for item_id, _ in sync_recommendations]
            extended_exclude = list(exclude_items) + sync_ids

            all_individual_recs = self.single_user_recommender.recommend_individual_next_top_k(remaining, extended_exclude)
            for user_id in self.users_ids:
                next_round_recs[user_id] = next_round_recs[user_id] + all_individual_recs.get(user_id, [])

        logging.info(f"first round total: { {uid: len(recs) for uid, recs in next_round_recs.items()} }")
        return next_round_recs

    def _filter_only_unique_items(
        self,
        previous_round_votes: Dict[str, List[Vote]]
    ) -> Dict[str, List[Vote]]:
